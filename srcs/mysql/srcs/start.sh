...
USE mysql;
FLUSH PRIVILEGES ;
GRANT ALL ON *.* TO 'root'@'%' identified by '$MYSQL_ROOTPASS' WITH GRANT OPTION ;
GRANT ALL ON *.* TO 'root'@'$WP_HOST' identified by '$MYSQL_ROOTPASS' WITH GRANT OPTION ;
GRANT ALL ON *.* TO 'root'@'localhost' identified by '$MYSQL_ROOTPASS' WITH GRANT OPTION ;
SET PASSWORD FOR 'root'@'localhost'=PASSWORD('${MYSQL_ROOTPASS}') ;
SET PASSWORD FOR 'mariadb.sys'@'localhost'=PASSWORD('${DB_SYSPASS}') ;
DROP DATABASE IF EXISTS test ;
CREATE DATABASE IF NOT EXISTS \`$WP_MYSQLDBNAME\` CHARACTER SET utf8 COLLATE utf8_general_ci;
GRANT ALL ON \`$WP_MYSQLDBNAME\`.* to '$WP_MYSQLUSER'@'%' IDENTIFIED BY '$WP_MYSQLPASS';
